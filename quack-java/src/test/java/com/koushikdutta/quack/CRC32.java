...
public class CRC32 {
	...
	/*
	 * this is the academic abc CRC code.. it works :) public static int
	 * calc(byte[] ba, int offset, int count) { int crc = 0xFFFFFFFF; // initial
	 * contents of LFBSR int poly = 0xEDB88320; // polynomial int temp;
	 * 
	 * for (int j = offset; j < count + offset; j++) { temp = (crc ^
	 * reverseByte(ba[j])) & 0xff;
	 * 
	 * // read 8 bits one at a time for (int i = 0; i < 8; i++) { if ((temp & 1)
	 * == 1) temp = (temp >>> 1) ^ poly; else temp = (temp >>> 1); } crc = (crc
	 * >>> 8) ^ temp; } return reverseInt(crc); }
	 */
	...
	public static void makeTable() {
		int poly = 0xEDB88320; // MPEG-2 CRC polynomial
		int crc;
